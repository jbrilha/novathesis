...
\subsection{Como se pretende resolver cada desafio}
...
\todo{FIXED: BLE/LORA mismatch with TCP}
...
It's 
\todo{FIXED: addressed dupes and bridges}
immediately apparent that -- if these exchanges aren't properly integrated
...
(e.g., by peeking at the sender's ID).
Nodes supporting multiple protocols naturally serve as bridges, enabling
topology formation and message forwarding between nodes that might not share
the same protocol stack, this is exemplified in Figure~\ref{fig:something}.
...
structure of heterogeneous disaster architecture, \textbf{\gls{ubabel}} will
...
\begin{description}
	...
	\item \textbf{Threat model:} The system will prioritize availability and
	      ...
	      \begin{itemize}
              ...
              \item \textbf{Local adversary}: attackers can monitor/disrupt
                  local regions but not entire network simultaneously
          ...
          attacks. If an adversary with sophisticated capabilities attacks
          during a disaster, communication infrastructure would already be
          their target regardless of security mechanisms.
\end{description}
