...
\section{Recap}
\label{sec:cha3_recap}

\subsection{The Problem}

Current \gls{iot} and domotics systems assume continuous infrastructure
availability, an assumption that proves rather catastrophic during
disasters, in hazardous environments, or when privacy requirements demand
local autonomy (Chapter~\ref{cha:introduction}).
...
\paragraph{Centralized coordination.} Protocol selection requires master nodes
or SDN controllers. Topology management depends on gateway coordinators. Time
synchronization relies on master beacons. Authentication requires persistent
servers or group leaders. When these central elements fail or become
unreachable, coordination collapses despite local devices remaining
operational.

\paragraph{Single-protocol operation.} Gossip-based synchronization assumes
homogeneous networks where all nodes share identical communication
characteristics. Compression techniques optimize for single technologies
without considering protocol-specific energy costs. Topology maintenance
protocols assume either persistent connections or uniform radio properties
ill-suited for multi-protocol embedded platforms.
...
No existing work integrates solutions to these challenges into a unified
framework for resource-constrained devices targeting autonomous operation
