...
brought into the Pico platform, so by using it as a common execution runtime,
...
thin \glspl{HAL}, enabling re-utilization of higher-lever protocols and
coordination logic across Class-1 devices.

\todo{mencionar anywhere que usamos lwip? custom lora drivers, etc?}

\subsection{Implementation Language}

The \gls{ubabel} framework is implemented in the C programming language. This
is the language of choice for the target development platforms, as it allows
precise control over memory usage, interrupt behavior, and timing. These
properties are essential in embedded environments with limited memory strict
