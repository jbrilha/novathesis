...
radio/wireless technologies. We distinguish between \emph{core components}, and
...
(2) Delivering events to all registered subscribers for publish/subscribe
style communication;
...
Short-range radios like \gls{BLE}, \gls{ESP-NOW}, and
...
(2) Advertising local device capabilities (supported radio interfaces, computational resources, battery status);
...
survive infrastructure failures. It has two modes of operation: when
infrastructure is available (e.g., Wi-Fi access points), it builds and
maintains an overlay topology that may span beyond direct neighbors; when
infrastructure fails, the topology gracefully degrades to proximity-based
membership using direct wireless links discovered by the Peer Discovery
Service. \todo{FIXED: dizer que exploramos heterogeneidade para resilience}The
topology manager can exploit neighbor heterogeneity (e.g., available radio
technologies, distance, processing capabilities) to enhance overlay resilience
through diversified connectivity.
