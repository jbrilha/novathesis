...
% \todo{extend initial paragraph, more current context - what's the state of the
% art, how it's done and addressed, how it can be a problem}
...
facilities -- with the global IoT market projected to reach 40 billion
connected devices by 2030\cite{sinha2024stateofiot}, and add between \$14 and
\$15 trillion to the global GDP by the same year.~\cite{thierer2015projecting}.
\todo{too much money talk? also, artigo de 2015, maybe menos relevante?}

However, current IoT systems remain fundamentally dependent on continuous
Internet connectivity and centralized cloud infrastructures. The dominant
architectural pattern across the industry involves edge devices collecting data
and transmitting it to remote cloud servers for processing, storage, and other
control logic. This approach delivers scalability and ease of management
but creates a fundamental dependency on network availability.

While this cloud-centric model has enabled rapid IoT adoption, it introduces
critical vulnerabilities across scenarios where continuous connectivity cannot
be guaranteed.\\

Recent infrastructure failures illustrate the fragility of cloud-dependent
architectures: the AWS US-EAST-1 outage in October 2025 disrupted services from
banking to smart homes worldwide, demonstrating how a DNS configuration error
in a \textit{single region} can cascade into global disruptions.
\todo{exemplo mais concreto ou isto basta?}
...
latency, redundant cloud regions for availability, and hybrid architectures
that combine local and remote processing.\todo{citations here!!}
...
\begin{description}
    \item \todo{expandir estes também ou short and to the point é melhor?}
...
critical implications in other contexts and \texttt{MicroBabel} addresses such
...
critical limitations: increased latency from round-trip communication to distant
servers, reduced availability during network disruptions or cloud outages, and
poor fault-tolerance when infrastructure fails.
...
resilient multi-tier architecture. \todo{is this too repetitive considering the
paragraph before the research topics?}
