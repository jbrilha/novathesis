...
\begin{description}
    ...
    \todo{tive de meter as labels inline para o final do chapter não passar para a próxima página... se ficar feio assim mudo algo elsewhere}
    ...
    \item[\textbf{Lack of autonomous operation:}] Device deployment and
        ...
        node failures in the \gls{iot} infrastructure, which is naturally
        susceptible to network failures and intermittent connectivity;
...
dependency has critical implications in other contexts and \gls{ubabel}
addresses such scenarios across scales: from residential systems requiring
local control, to building monitoring infrastructures that must operate during
emergencies, to disaster response networks where autonomous operation becomes
essential when traditional infrastructure fails.
...
remote locations. \gls{ubabel} will address this by leveraging a
...
\begin{description}
	\item[\textbf{Chapter~\ref{cha:related_work}}] Reviews related work in
	      \gls{P2P} networking, multi-protocol communication, decentralized
	      synchronization, data compression, and security.\todo{retirar security daqui altogether?}
