...
While cloud platforms can benefit \gls{iot} deployments through additional
...
\begin{description}
    ...
    \item \textbf{Lack of autonomous operation}\\
        ...
        tolerance for individual node failures in the \gls{iot} infrastructure, which
...
\todo{fazer distinção maior entre iot e domotics? fiz no related work, devia mudar para aqui?}
While these challenges affect both \gls{iot} and domotics systems alike -- from
...
critical implications in other contexts and
\todo{o que achamos do rebrand MicroBabel -> $\mu$Babel?}
\textbf{\gls{ubabel}} addresses such scenarios across scales: from residential
systems requiring local control, to building monitoring infrastructures that
must operate during emergencies, to disaster response networks where
autonomous operation becomes essential when traditional infrastructure fails.
...
In this document we introduce \textbf{\gls{ubabel}}, a lightweight framework
...
autonomously during infrastructure failures. \textbf{\gls{ubabel}} will
integrate with the broader Babel Ecosystem, which runs on hardware with greater
resources such as full Raspberry Pi boards or computers, enabling a class-based
architecture where resource-constrained edge devices can seamlessly
interoperate with computational nodes for data aggregation, processing, and
coordination.
...
technologies (\gls{BLE}, \gls{LoRa}, Wi-Fi, etc.) that have distinct
...
resilience; multi-channel operation provides frequency diversity for
anti-jamming and throughput.
...
Traditional \gls{iot} deployments rely heavily on Wi-Fi access points, cellular
...
\textbf{\gls{ubabel}} addresses this by leveraging a multi-protocol
communication approach, supporting a diverse protocol stack (\gls{BLE},
\gls{LoRa}, ESP-NOW, ZigBee) that can operate independently of infrastructure.
\todo{add glossary entries for esp-now, zigbee, etc?}
...
\gls{iot} deployments naturally comprise devices with varying capabilities, from
...
\textbf{\gls{ubabel}} exploits it through capability-aware protocols that allow
...
can be deployed rapidly without extensive configuration. \textbf{\gls{ubabel}} 
...
%
% This dual-mode operation demonstrates how \textbf{\gls{ubabel}} systems can
...
\todo{"main" aqui implica outras contributions secundárias, if that's the case
não devia inclui-las também?}
contributions:

\begin{itemize}
    \item A decentralized architecture supporting multiple communication
        protocols (\gls{BLE}, \gls{LoRa}, ZigBee, ESP-NOW) with adaptive
        switching based on \gls{QoS} requirements, resource availability and
        device capabilities;
