...
Their evaluation comparing MQTT, CoAP, and HTTP revealed a significant
discovery: receivers consistently consume 15-20\% more energy than senders
across all protocols, shifting the energy bottleneck from endpoint sensors to
intermediary nodes like gateways. This receiver energy asymmetry has direct
implications for gateway power management in disaster scenarios where battery
replacement becomes impossible. The framework achieved 34\% energy reduction
versus static HTTP, 23\% versus static MQTT, and 18\% versus static CoAP, while
requiring 85\% less computation than machine learning approaches.
...
energy expenditure of these different approaches, and grants useful
foundations for more complex multi-protocol approaches, in particular with the
multi-criteria decision framework and hysteresis control mechanism, but the
actual system cannot operate when infrastructure fails.\\
...
These limitations are addressed through opportunistic multi-protocol operation
(BLE + Wi-Fi + LoRa + ESP-NOW) wherein devices adaptively select communication
mediums based on factors like message priority, neighbor availability, energy
budget, network conditions and device capabilities.\todo{nail down a
arquitetura primeiro maybe}

The system employs decentralized protocol selection without requiring SDN
controllers or infrastructure coordination, instead using local decision-making
informed by neighborhood information gathered through gossip protocols. 
...
The work on Resilient Edge-enable IoT~\cite{resilient2019} addresses coordinator
failures through dynamic leader election and backup mechanisms within their
framework.
...
The proposed RGCS employs a convege-to-max criterion rather than average-value
