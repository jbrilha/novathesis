...
occupancy) and classroom occupancy:
...
(4) Optional cloud integration (\gls{NFR}, presented in Section~\ref{subsubsec:nonfunc_reqs}).
...
This mode validates:
(1) Infrastructure independence (\gls{FR}, presented in Section~\ref{subsubsec:func_reqs});
...
aggregation tier autonomously computes evacuation routes (with building layout
information in local storage or established dynamically during normal
operation) and updates actuation tier displays which show real-time guidance.
Critical functions remain operational without coordination tier.
...
out in the coming months, depicted in Figure~\ref{fig:gantt_chart}:
