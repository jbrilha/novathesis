...
gossip-based synchronization for wireless sensor networks.\todo{mau usar WSN aqui? é o mencionado nos papers}
...
known peers also provide a \emph{catalog} with potential synchronization
partners beyond their immediate active neighbors. When a node receives a
shuffle message containing peer descriptors (IDs and capabilities), it can
evaluate these peers as candidates for time synchronization based on their
advertised characteristics -- for instance, prioritizing peers with \gls{NTP}
access or those maintaining more reliable clock sources.
\todo{FIXED: quantum leap? mencionar aqui GPS clock sync como possibilidade?}
