...
A core pillar of resilient and decentralized \glspl{ISE} is the
...
Application-layer communication protocols typically focus on interoperability
and efficient data exchange under stable connectivity assumptions rather than
autonomous operation during infrastructure loss. At this layer, two messaging
protocols stand out given their wide-spread adoption in \gls{iot} deployments:
...
by \gls{ubabel}, and emphasize the contrast between higher-layer protocols
which tend to optimize for usage under stable infrastructure, and resilience
requirements that push architectural decisions towards lower layers.

For this reason, while such protocols are of interest for broader
interoperability (see \glspl{NFR} in Section~\ref{subsubsec:nonfunc_reqs}),
they are not a part of the foundational protocols employed by our work.

\subsection{Physical- and Link-Layer Technologies}

This section reviews the physical- and link-layer wireless technologies
relevant to resilient and decentralized \gls{ISE} deployments, as they are
commonly adopted due to their low energy consumption and plentiful hardware
availability. While the previous section addressed higher-layer messaging
protocols, the technologies discussed here provide the foundational
communication stack that determines the behavior of \gls{ubabel}.
...
\paragraph{Energy Considerations:} Wireless protocol selection must account for
energy costs: Wi-Fi consumes significantly more power than \gls{BLE}, while
\gls{LoRa} power consumption depends on \gls{SF} configuration.\\

Together, these characteristics illustrate the inherent trade-offs
faced by resilient \gls{ISE} deployments operating without reliable
...
complementary features of each technology and adapt communication strategies to
...
multi-radio operation and architectural choices discussed in
