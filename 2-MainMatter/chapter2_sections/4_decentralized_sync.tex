...
\todo{FIXED: moved NTP and GPS talk to discussion}
...
integration. 
\todo{FIXED: aqui}
One example would be when a node receives a shuffle message
containing peer descriptors (IDs and capabilities): it can evaluate these peers
as candidates for time synchronization based on their advertised
characteristics -- for instance, prioritizing peers with access to \gls{NTP},
\gls{GPS} clock synchronization, or simply those maintaining more reliable
clock sources.
\\
