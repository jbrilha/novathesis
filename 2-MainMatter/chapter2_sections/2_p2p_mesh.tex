...
discussed in Section~\ref{subsec:multi-channel} employs RPL for its \gls{WSN}
layer, demonstrating practical routing in resource-constrained disaster
scenarios.
...
\label{subsec:microbabel_implications_2}
...
TCP availability and persistent connections is not suitable for embedded
devices using connectionless radio technology or operating under intermittent
...
No existing work integrates connectionless neighbor discovery, hybrid view
maintenance, true P2P routing (not tree-based), and partition-tolerant
store-and-forward on resource-constrained embedded platforms.

\textbf{\gls{ubabel}}'s approach (Section \ref{subsec:microbabel_approach_2})
...
heavyweight middleware platforms or HyParView's \gls{TCP} requirements exceed
embedded device capabilities.

\textbf{\gls{ubabel}} intends to address these limitations through a fully
...
\inlinetodo{\gls{BLE}, \gls{LoRa}...?} rather than \gls{TCP} connections,
accommodating the connectionless nature of embedded radio/wireless
communication.

Neighbor discovery combines \inlinetodo{\gls{BLE}, LoRa...?} advertising beacons
...
This multi-mechanism approach mitigates potential range limitations: while \gls{BLE}
...
Unlike the tree topology of Multi-Protocol Gateway or the \gls{RPL} \gls{DODAG} structure
of heterogeneous disaster architecture, \textbf{\gls{ubabel}} will implement true
\gls{P2P} mesh networking where any node can communicate with any other node
...
and gossip: link quality metrics (\gls{RSSI}, \gls{PLR}), neighbor
...
(Section~\ref{sec:multi_protocol}), rather than centralized allocation of roles.
